    ...
    {foreach $images img}
    	<li class="rounded" style="width:{$li_width}px;height:{$li_height}px;background-color:{$bg_settings.thumb_bgcolor};">
    		<a style="line-height:{$settings.thumb_height}px;" rel="prettyPhoto[pp_gal]" href="{$BASE_URL}{$current_path}/{$img.file_name}" title="{if $img.description}{$img.description}{else}{$img.file_name}{/if}">
